...
const uniswapTokenObj: { [key: string]: { [key: string]: typeof uniswapToken.tokens[0] } } = {};
...
export function getUniswapTokenObjByAddress(chainId: number, address: string) {
    return Object.values(uniswapTokenObj[chainId]).find((token) => token.address === address);
}

export function getToken(name: string): Token {
    const chainId = getChainId();
    const tokenObj = getUniswapTokenObjByName(chainId, name);
    return new Token(chainId, tokenObj.address, tokenObj.decimals, tokenObj.symbol, tokenObj.name);
}

export function getTokenByAddress(address: string): Token | null {
    const chainId = getChainId();
    const tokenObj = getUniswapTokenObjByAddress(chainId, address);
    return tokenObj ? new Token(chainId, tokenObj.address, tokenObj.decimals, tokenObj.symbol, tokenObj.name) : null;
}

export function findPoolAddress(token0: Token, token1: Token): string {
    return POOL_ADDRESSES[`${token0.symbol}_${token1.symbol}`] || POOL_ADDRESSES[`${token1.symbol}_${token0.symbol}`];
}
