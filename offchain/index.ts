    ...
    for (const [pool0, pool1, pool2] of swapPaths) {
        try {
            ...
            if (profitAmount.greaterThan(0)) {
                profitableSwapPaths.push({ amountIn, profitAmount, pool0, pool1, pool2 });
            }
        } catch (e) {
...
function displayProfit({ amountIn, profitAmount, pool0, pool1, pool2 }: ProfitableSwapPath) {
    console.log(`-----------------`);
    console.log(
        `[${pool0.token0.symbol}, ${pool0.token1.symbol}] [${pool1.token0.symbol}, ${pool1.token1.symbol}] [${pool2.token0.symbol}, ${pool2.token1.symbol}]`
    );
    console.log(`pool0: ${pool0.id} tokens [${pool0.token0.id} ${pool0.token1.id}]`);
    console.log(`pool1: ${pool1.id} tokens [${pool1.token0.id} ${pool1.token1.id}]`);
    console.log(`pool2: ${pool2.id} tokens [${pool2.token0.id} ${pool2.token1.id}]`);
    console.log(`Input Amount: ${amountIn.toExact()} ETH`);
    console.log(`Profit: ${profitAmount.toExact()} ETH (~ ${profitAmount.multiply(1000).toFixed(2)} USD)`);
    console.log(
        `Diff: ${profitAmount
            .multiply(Math.pow(10, WETH.decimals))
            .multiply(100)
            .divide(CurrencyAmount.fromRawAmount(WETH, ethers.utils.parseUnits('1', WETH.decimals).toString()))
            .toFixed(4)}%`
    );
}
...
async function main() {
    ...
    flat.sort((a, b) => a.profitAmount.toExact().localeCompare(b.profitAmount.toExact())).map(displayProfit);
