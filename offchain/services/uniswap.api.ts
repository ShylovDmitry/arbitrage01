...
async function getTicksForPool(poolId: string, ticksFirst: number, ticksSkip: number): Promise<UniswapApiTick[]> {
...
async function getPools(first: number, skip: number): Promise<UniswapApiPool[]> {
    const response = await axios({
        url: endpoint,
        method: 'post',
        data: {
...
export async function getAllPools(): Promise<UniswapApiPool[]> {
    const limit = 100;
    const result = await Promise.all([...Array(5).keys()].map((val) => getPools(limit, val * limit)));
    return result.flat();
}
