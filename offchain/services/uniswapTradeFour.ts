...
import { createPool, generateFourSwapPaths } from "./uniswap";
import { uniswapTrade } from "../../onchain/src/uniswapTrade";
import { ProfitableFourSwapPath } from "../interfaces/profitableFourSwapPath";

const WETH = getToken("WETH");

async function getProfitableFourSwapPaths(
  amountInEth: number,
  swapPaths: [UniswapApiPool, UniswapApiPool, UniswapApiPool, UniswapApiPool][]
): Promise<ProfitableFourSwapPath[]> {
  const amountIn = CurrencyAmount.fromRawAmount(
    WETH,
    ethers.utils.parseUnits(amountInEth.toString(), WETH.decimals).toString()
  );

  const profitableSwapPaths: ProfitableFourSwapPath[] = [];
  for (const [pool0, pool1, pool2, pool3] of swapPaths) {
    try {
      ...
      const [amountOut3] = await createPool(pool3).getOutputAmount(amountOut2);

      const profitAmount = amountOut3.subtract(amountIn);
...
function displayFourProfit({
  ...
  pool2,
  pool3,
}: ProfitableFourSwapPath) {
  console.log(`-----------------`);
  console.log(
    `[${pool0.token0.symbol}, ${pool0.token1.symbol}] [${pool1.token0.symbol}, ${pool1.token1.symbol}] [${pool2.token0.symbol}, ${pool2.token1.symbol}] [${pool3.token0.symbol}, ${pool3.token1.symbol}]`
  );
  console.log(
  ...
  console.log(
    `pool3: ${pool3.id} tokens [${pool3.token0.id} ${pool3.token1.id}] fee ${pool3.feeTier}`
  );
...
export async function tradeFour(minProfitAmountEth: string) {
  ...
  const swapPaths = await generateFourSwapPaths(pools);
  ...
  const profitableSwapsArray = await Promise.all([
    // await getProfitableFourSwapPaths(0.1, swapPaths),
    // await getProfitableFourSwapPaths(0.15, swapPaths),
    // await getProfitableFourSwapPaths(0.3, swapPaths),
    // await getProfitableFourSwapPaths(0.4, swapPaths),
    await getProfitableFourSwapPaths(0.5, swapPaths),
    await getProfitableFourSwapPaths(1, swapPaths),
    await getProfitableFourSwapPaths(1.5, swapPaths),
    await getProfitableFourSwapPaths(2, swapPaths),
    // await getProfitableFourSwapPaths(2.5, swapPaths),
    // await getProfitableFourSwapPaths(3, swapPaths),
  ]);
